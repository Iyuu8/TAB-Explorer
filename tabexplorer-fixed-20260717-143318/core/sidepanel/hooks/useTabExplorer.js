...
export function useTabExplorer() {
  ...
  const childFolders = useCallback((parentId) => wsFolders.filter((f) => f.parentId === parentId), [wsFolders])
  ...
  const globalSearchResults = useMemo(() => {
    ...
    const matchedFolders = folders.filter((f) => matchesSearch(f.name))
  ...
  }, [query, folders, links, matchesSearch])
  ...
  function createFolder(name, parentId) {
    ...
    pushHistory()
    const now = Date.now()
    const folder = { id: makeId(), workspaceId: activeWorkspaceId, parentId: parentId || null, name: name.trim() || "New Folder", color: "", createdAt: now, updatedAt: now }
  ...
  return {
    ...
    wsFolders, wsLinks, childFolders, childLinks, countUnder, matchesSearch, query,
    ...
    toast, setToast,
    // crud
    createWorkspace, createFolder, createLink, updateFolderColor, updateLinkDetails, updateWorkspaceName, deleteItems, deleteOne, deleteWorkspace,
    setActiveWorkspaceId, setExpanded,
    // tabs
    saveTabs, requestOpen, requestOpenMultiple, collectLinksForOpen, openTabsBatched,
    // undo
    undo, historyCount
  }
}
